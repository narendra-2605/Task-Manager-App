...
export const getTodos = () => async (dispatch) => {
    const response = await axios.get(`http://localhost:3002/allTodos/${userId}`);
    dispatch({
        type: GET_TODOS,
...
export const addNewTodo = (todo) => (dispatch) => {
    ...
    const response = axios.post("http://localhost:3002/postTodo", todo);
...
export const updateTodo = (id, todo) => (dispatch) => {
    const response = axios.put(`http://localhost:3002/updateTodo/${id}`, todo);
    if (response) {
...
export const deleteUserTodo = (id) => async (dispatch) => {
    try {
        ...
        const userId = JSON.parse(localStorage.getItem("user"))["data"]["user"]["_id"];
        console.log(userId);
        // const response = await axios.post(`http://localhost:3002/deleteTodo/${id}`, { id: userId });
        const response = await axios.delete(`http://localhost:3002/deleteTodo/${id}/${userId}`);
        console.log("response delet user todo", response);
        dispatch({
